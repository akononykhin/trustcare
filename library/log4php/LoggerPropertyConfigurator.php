...
class LoggerPropertyConfigurator implements LoggerConfigurator {
    ...
    function &parseAppender($props, $appenderName)
    {
        ...
        /* 11.03.2009 ...*/
        $filterClass = @$props[$filterPrefix];
        $filterClass = LoggerOptionConverter::substVars($filterClass, $props);
        if(!is_null($filterClass)) {
            if (!class_exists($filterClass, false)) { /* to prevent autoload if enabled */
                @include_once(LOG4PHP_DIR . "/varia/{$filterClass}.php");
            }
            if (class_exists($filterClass)) {
                $filter = new $filterClass();
                LoggerPropertySetter::setPropertiesByObject($filter, $props, $filterPrefix . ".");
                $filter->activateOptions();
                $appender->addFilter($filter);
            } else {
                LoggerLog::warn("LoggerPropertyConfigurator::parseAppender() FILTER. class '$filterClass' doesnt exist");
            }
