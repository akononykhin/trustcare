 ...
 * @version    SVN: $Id: Logger.php 556801 2007-07-17 03:36:21Z carnold $
...
/* --------------------------------------------------------------------------*/
    ...
    public function callAppenders($event) 
    {
        ...
        if (sizeof($this->aai) > 0) {
            foreach (array_keys($this->aai) as $appenderName) {
                ...
                $appenders[$appenderName] = $this->aai[$appenderName];
            }
        }
        if ($this->parent != null && $this->getAdditivity() && $event->getLevel()->isGreaterOrEqual($this->parent->getEffectiveLevel())) {
            $parentAppenders = $this->parent->callAppenders($event);
            $appenders = array_merge($appenders, $parentAppenders);
        }
        return $appenders;
    }
    ...
    public function forcedLog(&$fqcn, &$caller, &$level, &$message)
    {
        ...
        $appenders = $this->callAppenders($event);
