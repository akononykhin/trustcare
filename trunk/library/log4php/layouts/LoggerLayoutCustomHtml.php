 ...
 * @subpackage layouts
 */
class LoggerLayoutCustomHtml extends LoggerLayout {
    ...
    function format($event)
    {
        ...
        if ($event->getLevel()->isGreaterOrEqual(LoggerLevel::getLevelError())) {
            ...
            for($i = 0; $i < count($traceStack); $i++) {
                $info = sprintf("\t\t#%d: %s (%s, %s)", $i, $traceStack[$i]['file'], $traceStack[$i]['line'], $traceStack[$i]['function']);
                $sbuf .= htmlentities($info, ENT_QUOTES) . '<br/>' . LOG4PHP_LINE_SEP;
            }
        }
        $sbuf .= "</td>" . LOG4PHP_LINE_SEP;
