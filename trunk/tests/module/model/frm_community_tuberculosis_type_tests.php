...
class TestOfFrmCommunityTuberculosisType extends UnitTestCase {
    ...
    function setUp() {
        ...
        try {
            $fileName = sprintf("%s/_files/frm_community_tuberculosis_type_test.sql", dirname(__FILE__));
            $fh = fopen($fileName, "r");
            if ($fh) {
                while (!feof($fh)) {
            ...
            $params = array(
                'id' => $this->db->nextSequenceId('frm_community_tuberculosis_type_id_seq'),
                'id_frm_community' => 1,
            ...
            $query = sprintf("insert into frm_community_tuberculosis_type(%s) values(%s);", join(",", $columns), join(",", $values));
            $res = $this->db->query($query);
    ...
    function tearDown() {
        $query = sprintf("delete from frm_community_tuberculosis_type;");
        $this->db->query($query);
        
        $query = sprintf("update db_sequence set value=1 where name='frm_community_tuberculosis_type_id_seq';");
        $this->db->query($query);
    ...
    function testInitializing() {
        ...
        $model = new TrustCare_Model_FrmCommunityTuberculosisType($params);
    ...
    function testLoadUnexistingById() {
        $model = TrustCare_Model_FrmCommunityTuberculosisType::find(-1 * $this->paramsAtDb['id'], array('mapperOptions' => array('adapter' => $this->db)));
    ...
    function testSaveNew() {
        ...
        try {
            ...
            $model1 = TrustCare_Model_FrmCommunityTuberculosisType::find($params['id'], array('mapperOptions' => array('adapter' => $this->db)));
    ...
    function testChangeParameters() {
        ...
        if(!is_null($model)) {
            ...
            try {
                ...
                $model1 = TrustCare_Model_FrmCommunityTuberculosisType::find($this->paramsAtDb['id'], array('mapperOptions' => array('adapter' => $this->db)));
    ...
    /**
     * 
     * @param TrustCare_Model_FrmCommunityTuberculosisType $model
     * @param array $params
     * @return void|void
     */
