...
class TestOfFacilityType extends UnitTestCase {
    ...
    function setUp() {
        ...
        try {
            $params = array(
                'id' => $this->db->nextSequenceId('facility_type_id_seq'),
                'name' => 'Test1',
            );
            ...
            $query = sprintf("insert into facility_type(%s) values(%s);", join(",", $columns), join(",", $values));
    ...
    function tearDown() {
        ...
        $query = sprintf("update db_sequence set value=1 where name='facility_type_id_seq';");
        $this->db->query($query);
    }
    ...
    function testInitializing() {
        ...
        $model = new TrustCare_Model_FacilityType($params);
    ...
    function testLoadExistingById() {
        $model = TrustCare_Model_FacilityType::find($this->paramsAtDb['id'], array('mapperOptions' => array('adapter' => $this->db)));
        
        $this->_compareObjectAndParams($model, $this->paramsAtDb);
    }

    function testLoadUnexistingById() {
        $model = TrustCare_Model_FacilityType::find(-1 * $this->paramsAtDb['id'], array('mapperOptions' => array('adapter' => $this->db)));
    ...
    function testSaveNew() {
        ...
        try {
            $model = new TrustCare_Model_FacilityType(array('mapperOptions' => array('adapter' => $this->db)));
            ...
            $model1 = TrustCare_Model_FacilityType::find($params['id'], array('mapperOptions' => array('adapter' => $this->db)));
    ...
    function testChangeParameters() {
        ...
        if(!is_null($model)) {
            ...
            try {
                ...
                $model1 = TrustCare_Model_FacilityType::find($this->paramsAtDb['id'], array('mapperOptions' => array('adapter' => $this->db)));
                $this->_compareObjectAndParams($model1, $params);
            }
            catch(Exception $ex) {
