...
class TestOfFrmCommunityStiType extends UnitTestCase {
    ...
    function setUp() {
        ...
        try {
            $fileName = sprintf("%s/_files/frm_community_sti_type_test.sql", dirname(__FILE__));
            $fh = fopen($fileName, "r");
            if ($fh) {
                while (!feof($fh)) {
            ...
            $params = array(
                'id' => $this->db->nextSequenceId('frm_community_sti_type_id_seq'),
                'id_frm_community' => 1,
            ...
            $query = sprintf("insert into frm_community_sti_type(%s) values(%s);", join(",", $columns), join(",", $values));
            $res = $this->db->query($query);
    ...
    function tearDown() {
        $query = sprintf("delete from frm_community_sti_type;");
        $this->db->query($query);
        
        $query = sprintf("update db_sequence set value=1 where name='frm_community_sti_type_id_seq';");
        $this->db->query($query);
    ...
    function testInitializing() {
        ...
        $model = new TrustCare_Model_FrmCommunityStiType($params);
    ...
    function testLoadExistingById() {
        $model = TrustCare_Model_FrmCommunityStiType::find($this->paramsAtDb['id'], array('mapperOptions' => array('adapter' => $this->db)));
        
        $this->_compareObjectAndParams($model, $this->paramsAtDb);
    }

    function testLoadUnexistingById() {
        $model = TrustCare_Model_FrmCommunityStiType::find(-1 * $this->paramsAtDb['id'], array('mapperOptions' => array('adapter' => $this->db)));
    ...
    function testSaveNew() {
        ...
        try {
            $model = new TrustCare_Model_FrmCommunityStiType(array('mapperOptions' => array('adapter' => $this->db)));
            ...
            $model1 = TrustCare_Model_FrmCommunityStiType::find($params['id'], array('mapperOptions' => array('adapter' => $this->db)));
    ...
    function testChangeParameters() {
        ...
        if(!is_null($model)) {
            ...
            try {
                ...
                $model1 = TrustCare_Model_FrmCommunityStiType::find($this->paramsAtDb['id'], array('mapperOptions' => array('adapter' => $this->db)));
                $this->_compareObjectAndParams($model1, $params);
            }
            catch(Exception $ex) {
    ...
    function testReplaceForFrmCommunity()
    {
        ...
        try {
            ...
            TrustCare_Model_FrmCommunityStiType::replaceForFrmCommunity($frmCommunityId, $newDictIds, array('mapperOptions' => array('adapter' => $this->db)));
    ...
    /**
     * 
     * @param TrustCare_Model_FrmCommunityStiType $model
     * @param array $params
     * @return void|void
     */
