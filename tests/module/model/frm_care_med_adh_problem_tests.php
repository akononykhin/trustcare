...
class TestOfFrmCareMedAdhProblem extends UnitTestCase {
    ...
    function setUp() {
        ...
        try {
            $fileName = sprintf("%s/_files/frm_care_med_adh_problem_test.sql", dirname(__FILE__));
            $fh = fopen($fileName, "r");
            if ($fh) {
                while (!feof($fh)) {
            ...
            $params = array(
                'id' => $this->db->nextSequenceId('frm_care_med_adh_problem_id_seq'),
                'id_frm_care' => 1,
            ...
            $query = sprintf("insert into frm_care_med_adh_problem(%s) values(%s);", join(",", $columns), join(",", $values));
            $res = $this->db->query($query);
    ...
    function tearDown() {
        $query = sprintf("delete from frm_care_med_adh_problem;");
        $this->db->query($query);
        
        $query = sprintf("update db_sequence set value=1 where name='frm_care_med_adh_problem_id_seq';");
        $this->db->query($query);
    ...
    function testInitializing() {
        ...
        $model = new TrustCare_Model_FrmCareMedAdhProblem($params);
    ...
    function testLoadUnexistingById() {
        $model = TrustCare_Model_FrmCareMedAdhProblem::find(-1 * $this->paramsAtDb['id'], array('mapperOptions' => array('adapter' => $this->db)));
    ...
    function testSaveNew() {
        ...
        try {
            $model = new TrustCare_Model_FrmCareMedAdhProblem(array('mapperOptions' => array('adapter' => $this->db)));
            ...
            $model1 = TrustCare_Model_FrmCareMedAdhProblem::find($params['id'], array('mapperOptions' => array('adapter' => $this->db)));
    ...
    function testReplaceForFrmCare() {
        ...
        try {
            ...
            TrustCare_Model_FrmCareMedAdhProblem::replaceForFrmCare($frmCareId, $newDictIds, array('mapperOptions' => array('adapter' => $this->db)));
