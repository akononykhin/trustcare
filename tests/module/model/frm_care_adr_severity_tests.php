...
class TestOfFrmCareAdrSeverity extends UnitTestCase {
    ...
    function setUp() {
        ...
        try {
            $fileName = sprintf("%s/_files/frm_care_adr_severity_test.sql", dirname(__FILE__));
            $fh = fopen($fileName, "r");
            if ($fh) {
                while (!feof($fh)) {
            ...
            $params = array(
                'id' => $this->db->nextSequenceId('frm_care_adr_severity_id_seq'),
                'id_frm_care' => 1,
            ...
            $query = sprintf("insert into frm_care_adr_severity(%s) values(%s);", join(",", $columns), join(",", $values));
            $res = $this->db->query($query);
    ...
    function tearDown() {
        $query = sprintf("delete from frm_care_adr_severity;");
        $this->db->query($query);
        
        $query = sprintf("update db_sequence set value=1 where name='frm_care_adr_severity_id_seq';");
        $this->db->query($query);
    ...
    function testInitializing() {
        ...
        $model = new TrustCare_Model_FrmCareAdrSeverity($params);
    ...
    function testLoadExistingById() {
        $model = TrustCare_Model_FrmCareAdrSeverity::find($this->paramsAtDb['id'], array('mapperOptions' => array('adapter' => $this->db)));
        
        $this->_compareObjectAndParams($model, $this->paramsAtDb);
    }

    function testLoadUnexistingById() {
        $model = TrustCare_Model_FrmCareAdrSeverity::find(-1 * $this->paramsAtDb['id'], array('mapperOptions' => array('adapter' => $this->db)));
    ...
    function testSaveNew() {
        ...
        try {
            $model = new TrustCare_Model_FrmCareAdrSeverity(array('mapperOptions' => array('adapter' => $this->db)));
            ...
            $model1 = TrustCare_Model_FrmCareAdrSeverity::find($params['id'], array('mapperOptions' => array('adapter' => $this->db)));
    ...
    function testChangeParameters() {
        ...
        if(!is_null($model)) {
            ...
            try {
                ...
                $model1 = TrustCare_Model_FrmCareAdrSeverity::find($this->paramsAtDb['id'], array('mapperOptions' => array('adapter' => $this->db)));
                $this->_compareObjectAndParams($model1, $params);
            }
            catch(Exception $ex) {
    ...
    /**
     * 
     * @param TrustCare_Model_FrmCareAdrSeverity $model
     * @param array $params
     * @return void|void
     */
