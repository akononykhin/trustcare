...
class Portal_Facility_TypeController extends ZendX_Controller_Action
{
    ...
    {
        $select = Zend_Registry::getInstance()->dbAdapter->select()->from("facility_type", array('count(id)'));
        ...
        if($this->getRequest()->isPost()) {
            if ($form->isValid($this->getRequest()->getPost())) {
                ...
                try {
                    ...
                    $checkModel = TrustCare_Model_FacilityType::findByName($name);
                    if(!is_null($checkModel)) {
                        $errorMsg = sprintf(Zend_Registry::get("Zend_Translate")->_("Facility Type '%s' has already been used"), $name);
                        throw new Exception("");
                    }
                    
                    $model = new TrustCare_Model_FacilityType();
                    $model->setName($name);
                    $model->save();
    ...
    public function editAction()
    {
        ...
        if(is_null($model)) {
            ...
            $this->_forward("message", "error", null, array('message' => Zend_Registry::get("Zend_Translate")->_("Unknown Facility Type")));
            return;
        }
